...
Each `.txt` contains only that cheat's block, so users can browse by cheat name and still get the correct file for their installed build.
...
Enable version fan-out for single-version cheats:
...
This is useful when Tinfoil likely under-labeled a cheat and you want the emulator folder tree to include every game version up front.
